...
config.General.workArea = 'HNL_data_2018'
config.section_('Data')
#config.Data.splitting = 'FileBased'
config.Data.splitting = 'LumiBased'
config.Data.publication = False
config.Data.outLFNDirBase = '/store/user/%s/HNL_data_2018' % (getUsernameFromSiteDB())
...
#    for dataset, infos in dataset_files.items():
    with open('input_data_2018.yml','r') as f:
       doc = yaml.load(f)
       ...
       for sample in samples:
         ...
         config.JobType.pyCfgParams = ['isMC=False', 'period=%s' % (period)]
