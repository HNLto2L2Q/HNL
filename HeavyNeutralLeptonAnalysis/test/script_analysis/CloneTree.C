...
int main(){
  ...
  infile_data.open("list_tree.txt");
  ...
  //  std::vector<std::string *> list_tree;

  //while(std::getline(infile_data,inputfilename_data)){
  //  list_tree.push_back(inputfilename_data);
  //}
  //infile_data.close();


  //oldtree->Add("Analysis_output_2GeV_all.root");

  //for(auto flag : list_tree){
  std::string name_tree;
  std::string str;
  while (std::getline(infile_data, str)){

    oldtree->Add(static_cast<TString>(str));
    name_tree="skimmed_" + str;
